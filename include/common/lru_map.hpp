...
class lru_map final {
 ...
 public:
  explicit lru_map(size_t capacity = DEFAULT_CAPACITY) : capacity_(capacity) {
    if (capacity_ == 0) {
  ...
  // Insert with move semantics for value
  ...
  // Insert with move semantics for both key and value
  void insert(Key&& key, Value&& value) {
    ...
    if (it != items_map_.end()) {
      ...
      items_list_.emplace_front(std::move(key), std::move(value));
      items_map_[items_list_.begin()->first] = items_list_.begin();
    }
  }

  // Generic emplace for perfect forwarding
  ...
  void emplace(const Key& key, Args&&... args) {
    ...
    if (it != items_map_.end()) {
      ...
      items_list_.emplace_front(std::piecewise_construct,
